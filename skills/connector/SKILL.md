...
state.json에서 `connector` 키의 `status`를 제거하고 `connected_services`는 **보존**한다 (기존에 연결된 서비스 정보 유지). `status`가 없으면 다음 `/connector` 실행 시 케이스 A(신규)로 돌아가서 처음부터 진행할 수 있다. `installing` 상태를 그대로 두면 케이스 B가 반복되는 무한 루프가 발생하므로 반드시 제거한다.
