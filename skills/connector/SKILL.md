...
1. pending_services에 있는 서비스들의 도구가 로드되었는지 ToolSearch로 확인한다.
   - **Amplitude 예외**: Amplitude는 OAuth 인증을 완료해야 도구가 로드된다. Amplitude가 pending_services에 있고 ToolSearch에 도구가 나타나지 않으면, Read 도구로 `~/.claude.json`에 amplitude MCP 설정이 존재하는지 확인한다. 설정이 존재하면 OAuth 인증이 아직 안 된 것이므로 아래 안내를 출력한다:
     ...
     5. 완료되면 "완료" 라고 말해주세요
     ```
     안내 출력 후 Amplitude 외 다른 pending_services가 있으면 먼저 연결 테스트를 진행한다. 다른 pending_services가 없으면 사용자의 "완료" 입력을 대기한다. 사용자가 "완료"라고 하면 Amplitude 연결 테스트를 이어서 진행한다.
...
> **Amplitude ⚠️ 인증 필요** 상태에서 Amplitude를 선택한 경우: MCP 설정 파일에 amplitude 항목이 이미 존재하므로 `claude mcp add` 명령을 다시 실행하거나 재시작할 필요가 없다. `/mcp` → amplitude → Authenticate → 브라우저 로그인만 진행하면 된다. amplitude.md의 "OAuth 인증" 절차만 따른다.
...
> **Amplitude 예외**: Amplitude를 ⚠️ 인증 필요 상태에서 선택하여 OAuth만 진행한 경우(신규 `claude mcp add`를 실행하지 않은 경우), Amplitude를 재시작이 필요한 서비스에 포함하지 않는다. 이 경우 Amplitude는 `/mcp` → Authenticate 후 즉시 연결 테스트가 가능하며, 재시작이 필요한 다른 MCP 서비스가 없다면 재시작 안내 자체를 건너뛴다.
...
> **Amplitude 테스트 시**: 가이드에 명시된 "테스트 전 안내" 메시지를 반드시 먼저 출력한다. OAuth 인증이 안 되어 있으면 도구가 로드되지 않으므로, `/mcp` → Authenticate를 먼저 안내해야 한다.
