...
5. 브라우저에서 Amplitude 로그인 페이지가 열린다.
...
4. 브라우저에서 Amplitude 로그인 페이지가 열린다.
...
**인증 완료 후** 테스트를 진행한다:
1. ToolSearch로 `+amplitude search` 검색 → 도구가 나타나는지 확인
2. 도구가 나타나면 `mcp__amplitude__search(query="a")` 호출로 실제 테스트 (쿼리 내용과 무관하게 인증 여부만 확인)
3. 성공하면: 조직의 콘텐츠 목록이 반환됨 → 연결 성공으로 판단하고 결과 요약 표시 (예: "N개 콘텐츠에 접근할 수 있습니다")
4. 결과가 빈 배열(`totalHits: 0`)이면: 연결 자체는 성공 — 조직에 콘텐츠가 없는 것이므로 연결 성공으로 표시
5. 오류가 반환되면: 아래 트러블슈팅 항목 확인 → 진전 없으면 [트러블슈팅 및 이슈 등록](troubleshoot.md) 절차 진행
