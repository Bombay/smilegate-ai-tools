...
> ⚠️ 토큰만 복사하세요. "Bearer "는 설정에서 자동으로 붙으므로 입력하지 않아도 됩니다.

## 토큰 입력

아래 메시지를 출력하고 다음 사용자 입력을 토큰으로 처리한다:

> 발급받은 BISKIT PAT 토큰을 아래에 붙여넣고 엔터를 눌러주세요.
> (발급을 아직 못 하셨으면 "다시 안내해줘"라고 말해주세요)

- 사용자가 토큰처럼 보이는 값을 입력하면 → 토큰으로 처리한다.
- 사용자가 "다시 안내해줘" / "모르겠어요" 등 발급 관련 텍스트를 입력하면 → 위의 BISKIT 토큰 발급 절차를 다시 안내하고, 완료 후 다시 토큰 입력을 요청한다.
