...
- "CLAUDE.md 다시 만들기" → state.json에서 `user_info`와 `role`을 불러와 Step 3의 생성 로직을 실행한다 (Step 2 정보 수집은 건너뛴다). CLAUDE.md가 이미 있으면 Step 3의 "이미 있는 경우" 분기를 동일하게 따른다.
- "필요없어요" → 종료
